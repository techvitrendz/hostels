	...
	try {
		...
		switch (img_reqd) {
			case "hostelList":
				...
				data = data.map((block) => ({
					...block,
					// normalize enums to lowercase for the frontend
					blockType: String(block.blockType).toLowerCase(),
		...
		// P1001 occurs when the client cannot reach the server
		if (
			typeof error === "object" &&
			error !== null &&
			"code" in error &&
		...
		// initialization errors (e.g. network unreachable) come with a different
		// class name, not a code.
		if (
			typeof error === "object" &&
			error !== null &&
			...
			return NextResponse.json(
				{ error: "Unable to initialize Prisma client (database unreachable)." },
				{ status: 503 },
			);
		}
